...
def depth(synset, prof_corrente_raggiunta,
    ...
    if (synset == 'entity.n.01'):  # controllo se con la chiamata corrente
        prof_corrente_raggiunta -= 1
...
def Leakcock_and_Chodorow(DEPTH_MAX, tutti_i_concetti_associati_a_w1, tutti_i_concetti_associati_a_w2):
    ...
    for coppia_concetto_synset_w1_corrente in tutti_i_concetti_associati_a_w1:
        for coppia_concetto_synset_w2_corrente in tutti_i_concetti_associati_a_w2:
            ...
            # sim(c1, c2) = sim_LC(s1, s2) = - log ( len(s1,s2) / (2*depth_MAX) ).
            ...
            len_synset_concetto_corrente_associato_a_w1_synset_concetto_corrente_associato_a_w2 = 0  # inizializzo len(s1,s2) = 0.
            ...
            #############################################################################################################################################################################
                ...
                synset_primo_antenato_comune = wn.synset('entity.n.01')
            ...
            # sim(c1,c2) = sim_path(c1,c2) = ( -log( len(s1,s2) / (2 * DEPTH_MAX) ) (Leackcock & Chodorow).
            # print("")
            ...
            if(sim_massima_gia_raggiunta):
                ...
                #per questa metrica che è -np.log(1/( (2 * DEPTH_MAX) + 1) ):
                return -np.log( 1/ ( (2 * DEPTH_MAX) + 1) ) #termine funzione Leakcock & Chodorow

            print("")
            print("numeratore: ", (len_synset_concetto_corrente_associato_a_w1_synset_concetto_corrente_associato_a_w2 + 1))
            print("denominatore: ", ( (2 * DEPTH_MAX) + 1) )
            leakcock_and_Chodorow_c1_c2 = -np.log( ( (len_synset_concetto_corrente_associato_a_w1_synset_concetto_corrente_associato_a_w2 + 1) / ( (2 * DEPTH_MAX) + 1) ) )
            ...
            print("leakcock_and_Chodorow_c1_c2 corrente: ", leakcock_and_Chodorow_c1_c2)

            # Ora controllo se cs_c1_c2 corrente è maggiore del max, perchè se così fosse allora devo aggiornare il max:
            if (leakcock_and_Chodorow_c1_c2 > max):
                max = leakcock_and_Chodorow_c1_c2  # aggiorno il max
...
with open('WordSim353.csv', 'r') as f:
    ...
    with open('Leackock & Chodorow.csv', 'w', encoding='UTF8', newline='') as file:
        ...
        for row in reader:
            if not columns_names:
                    ...
                    lc = Leakcock_and_Chodorow(DEPTH_MAX, tutti_i_concetti_associati_a_w1, tutti_i_concetti_associati_a_w2)
                    #print("lc max: ", lc)
