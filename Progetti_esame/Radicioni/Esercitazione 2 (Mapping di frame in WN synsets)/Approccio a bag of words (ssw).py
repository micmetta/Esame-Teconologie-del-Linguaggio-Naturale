...
########    In questa implementazione prima calcolare l'overlap NON CONSIDERO LE STOP WORDS:    ##########
...
def The_Lesk_Algorithm(word, sentence, tutti_i_sensi_della_parola_da_disambiguare): #sentence = Ctx_w
    ...
    for senso in tutti_i_sensi_della_parola_da_disambiguare:
        ...
        overlap = ComputeOverlap(signature, context) #context è una lista di parole mentre la signature conterrà la lista di liste degli esempi e la lista di liste delle glosse.
...
with open('annotazioni mie dei frames.csv', 'r', encoding='UTF8', newline='') as file:
    ...
    for row in reader:
        if not columns_names:
            ...
            #1.2) Addesso da tutti_i_concetti_associati_a_w prendo solo i synsets e li metto in un insieme che chiamo insieme_di_synsets_associati_a_w:
...
print("Accuratezza finale sistema: ", num_synsets_predetti_correttamente/num_synsets_predetti_in_totale) #0.416
print("")
