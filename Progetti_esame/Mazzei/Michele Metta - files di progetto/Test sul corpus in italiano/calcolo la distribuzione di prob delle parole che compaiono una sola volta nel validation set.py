...
#"parole che compaiono una sola volta nel Validation Set.txt"

#leggo le frasi dal validation set:
file_train = open("C:\\Users\\miky9\\PycharmProjects\\pythonProject\\Progetti TLN\\Mazzei\\Datasets BIO tagg\\it\\val.conllu", "r", encoding="utf8")
frasi_corpus = []
frase_corrente = ""
...
# with open('dizionario_num_parole_per_ogni_tag_nel_Development_Set.json', 'w', encoding='UTF-8') as json_file:
#     json.dump(dizionario_num_parole_per_ogni_tag, json_file)
#################################################################################################################


# file = open("parole che compaiono una sola volta nel Validation Set.txt", "w", encoding='UTF-8')
# file.write(parole_che_compaiono_una_sola_volta_nel_corpus)
# file.close()
'''

with open('dizionario_num_parole_per_ogni_tag_nel_Development_Set.json', 'r') as json_file:
    dizionario_num_parole_per_ogni_tag_nel_Development_Set = json.load(json_file)

file = open('parole che compaiono una sola volta nel Validation Set.txt', 'r', encoding='UTF-8')
...
print("num di parole che compaiono una sola volta: ", len(lista_parole_che_compaiono_una_sola_volta_nel_VS)) #21765
...
with open('prob_di_emissione_pre_calcolate_per_le_parole_sconosciute_assegnate_dalla_tecnica_che_utilizza_il_DS.json', 'w', encoding='UTF-8') as json_file:
    json.dump(probabilita_di_emissione_finali_assegnate_da_questa_tecnica_di_smoothing, json_file)
print("Il dizionario che contiene le prob. di emissione già calcolate è stato serializzato correttamente.")
##################################################################################################################################################################################