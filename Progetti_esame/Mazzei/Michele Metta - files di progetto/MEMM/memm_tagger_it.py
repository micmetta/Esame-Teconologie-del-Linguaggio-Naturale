...
def get_features(index, sequence, tag_index_1, data):
    ...
    features["UNIGRAM_%s" % sequence[index].lower()] = 1 #come prima features prende la parola corrente intera
    features["PREVIOUS_TAG_%s" % tag_index_1] = 1 #il tag assegnato alla parola precedente
    features["PREFIX_{0}".format(sequence[index]).lower()[:3]] = 1 #il prefisso della parola corrente (prime tre lettere della parola)
    features["SUFFIX_{0}".format(sequence[index]).lower()[-2:]] = 1 #il suffisso della parola corrente (ultime due lettere della parola)

    if remove_puncuation(sequence[index].lower().strip()).replace("s", "").isnumeric():
        features["NUMERIC"] = 1 #si segna se la word corrente è un numero

    # print("index (in get_features): ", index)
    # print("")
    if sequence[index].strip()[0].isupper():
        features["FIRST_UPPER"] = 1 #un'altra feature utilizzata è quella che dice se la word corrente inizia con la lettera maiuscola oppure no

    if index != 0:
        features["BIGRAM_{0}_{1}".format(sequence[index].lower(), sequence[index - 1].lower())] = 1 #se la word corrente non è la prima parola della frase prende come features sia la parola corrente che quella precedente
    else:
        features["FIRST_WORD_IN_SEQUENCE"] = 1 #si segna se la word corrente è la prima parola della frase

    if index != len(sequence) - 1:
        features["BIGRAM_{0}_{1}".format(sequence[index].lower(), sequence[index + 1].lower())] = 1 #se la word corrente non è l'ultima parola della frase prende come features sia la parola corrente che quella successiva
    else:
        features["LAST_WORD_IN_SEQUENCE"] = 1 #si segna se la word corrente è l'ultima parola della frase

    if index >= 1 and index < len(sequence) - 1:
        w1 = sequence[index].lower()
        w2 = sequence[index - 1].lower()
        w3 = sequence[index + 1].lower()
        features["TRIGRAM_{0}_{1}_{2}".format(w2, w1, w3)] = 1 #l' ultima feature utilizza un trigramma ovvero queste 3 parole: word corrente, word precedente, word successiva.
...
def load_data_test_set():
    ...
    frasi_test_con_tag = Estrazione_dati_BIO_tagging_Wikipedia_it.frasi_test_set_it() #prendo tutte le frasi di test con i rispettivi tags assegnati ad ogni parola.
...
def load_data():
    ...
    """
    Estrazione_dati_BIO_tagging_Wikipedia_it = importlib.import_module("Estrazione_dati_BIO_tagging_Wikipedia_it_e_en")
    frasi_train_con_tag = Estrazione_dati_BIO_tagging_Wikipedia_it.frasi_training_set_it_con_tag()
...
def test(log_reg, data):
    ...
    for tags_predetti_frase_corrente in tags_assegnati_a_tutte_le_frasi_di_test:
        for indice_tag_predetto_frase_corrente in range(0, len(tags_predetti_frase_corrente)):
            ...
            if(tags_predetti_frase_corrente[indice_tag_predetto_frase_corrente] == 0):
    ...
    print("")
    frasi_test_set = Estrazione_dati_BIO_tagging_Wikipedia_it_e_en.frasi_test_set_it() #estraggo tutte le frasi presenti nel test set con la funzione che ho creato.
